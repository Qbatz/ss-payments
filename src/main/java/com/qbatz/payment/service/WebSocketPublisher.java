...
public class WebSocketPublisher {
    ...
    public void sendUpdate(ZohoPaymentResponse payload) {
        if (payload.linkId() == null) {
            messagingTemplate.convertAndSend("/payments/" +
                    payload.paymentSessionId(), payload);
        }
        else {
            messagingTemplate.convertAndSend("/payments/" +
                    payload.linkId(), payload);
        }
    }

    public void sendCardUpdate(PaymentStatusCardType paymentStatusCardType) {
        messagingTemplate.convertAndSend("/payments/" +
                paymentStatusCardType.paymentLinkId(), paymentStatusCardType);
    }
}
